...
#ifndef AFFT_BIT_REVERSAL_PRIMITIVES_HPP
#define AFFT_BIT_REVERSAL_PRIMITIVES_HPP
...
namespace afft{
    ...
    // template<typename spec, std::size_t arr_size>
    ...
    //         spec::load(dest_arr[arr_size - 1], src_arr + offsets[arr_size - 1]);
    //         LoadOperandsWithOffset<spec, arr_size - 1>::call(dest_arr, src_arr, offsets); 
    //     }
    // };

    // template<typename spec>
    // struct LoadOperandsWithOffset<spec, 1>{
    //     using operand = typename spec::operand;
    //     using sample = typename spec::sample;
    //     static inline void call (operand* dest_arr, const sample* src_arr,  const std::size_t* offsets) {
    //         spec::load(dest_arr[0], src_arr + offsets[0]);
    //     }
    // };
    ...
    //         spec::store(dest_arr + offsets[arr_size - 1], src_arr[arr_size - 1]);
    //         StoreOperandsWithOffset<spec, arr_size - 1>::call(dest_arr, src_arr, offsets); 
    //     }
    // };

    // template<typename spec>
    // struct StoreOperandsWithOffset<spec, 1>{
    //     using sample = typename spec::sample;
    //     using operand = typename spec::operand;
    //     static inline void call (sample* dest_arr, const operand* src_arr,  const std::size_t* offsets) {
    //         spec::store(dest_arr + offsets[0], src_arr[0]);
    //     }
    // };
    ...
    //         constexpr std::size_t half_n_samples_per_operand = spec::n_samples_per_operand / 2;
    //         constexpr std::size_t half_arr_size = arr_size / 2;
    //         spec::interleave(arr_a[arr_size-2], arr_a[arr_size-1], arr_b[half_arr_size - 1], arr_b[half_arr_size - 1 + half_n_samples_per_operand]);
    //         Interleave<spec, arr_size - 2>::call(arr_a, arr_b);
    //     }
    // };

    // template<typename spec>
    // struct Interleave<spec, 2>{
    //     using sample = typename spec::sample;
    //     using operand = typename spec::operand;
    //     static inline void call (operand* arr_a, const operand* arr_b) {
    //         constexpr std::size_t half_n_samples_per_operand = spec::n_samples_per_operand / 2;
    //         spec::interleave(arr_a[0], arr_a[1], arr_b[0], arr_b[half_n_samples_per_operand]);
    //     }
    // };

    // template<typename spec>
    // struct Interleave<spec, 1>{
    //     using sample = typename spec::sample;
    //     using operand = typename spec::operand;
    //     static inline void call(operand* arr_a, const operand* arr_b) {
    //         // Do Nothing
    ...
    // template<typename spec, std::size_t arr_size, std::size_t interleave_factor>
    // struct ComputeTranspose{
    //     using sample = typename spec::sample;
    //     using operand = typename spec::operand;
    //     static inline void call (operand* arr_a, operand* arr_b) {
    //         Interleave<spec, arr_size>::call(arr_a, arr_b);
    //         ComputeTranspose<spec, arr_size, interleave_factor / 2>::call(arr_b, arr_a);
    //     }
    // };

    // template<typename spec, std::size_t arr_size>
    // struct ComputeTranspose<spec, arr_size, 1>{
    //     using operand = typename spec::operand;
    //     static inline void call (operand* arr_a, operand* arr_b) {
    //         Interleave<spec, arr_size>::call(arr_a, arr_b);
    //     }
    ...
    // template<typename spec, std::size_t alternate_factor>
    // struct AlternateOut{
    //     using operand = typename spec::operand;
    //     static inline const operand* call (const operand* arr_a, const operand* arr_b) {
    //         return AlternateOut<spec, alternate_factor / 2>::call(arr_b, arr_a);
    //     }
    // };

    // template<typename spec>
    // struct AlternateOut<spec, 1>{
    //     using operand = typename spec::operand;
    //     static inline const operand* call (const operand* arr_a, const operand* arr_b) {
    //         return arr_a;
    //     }
    ...
    // template<typename spec>
    // static inline void transpose_diagonal(typename spec::sample* out_real, typename spec::sample* out_imag, const typename spec::sample* in_real, const typename spec::sample* in_imag, const std::size_t* indexes) {
    //     using operand = typename spec::operand;
    ...
    //     LoadOperandsWithOffset<spec, n_samples_per_operand>::call(row_x, in_real, ind);
    //     LoadOperandsWithOffset<spec, n_samples_per_operand>::call(row_u, in_imag, ind);
    //     ComputeTranspose<spec, n_samples_per_operand, n_samples_per_operand>::call(row_y, row_x);
    //     ComputeTranspose<spec, n_samples_per_operand, n_samples_per_operand>::call(row_v, row_u);
    //     StoreOperandsWithOffset<spec, n_samples_per_operand>::call(out_real, AlternateOut<spec, n_samples_per_operand>::call(row_x, row_y), ind);
    //     StoreOperandsWithOffset<spec, n_samples_per_operand>::call(out_imag, AlternateOut<spec, n_samples_per_operand>::call(row_u, row_v), ind);
    // }

    // template<typename spec>
    // static inline void transpose_off_diagonal(typename spec::sample* out_real, typename spec::sample* out_imag, const typename spec::sample* in_real, const typename spec::sample* in_imag, const std::size_t* indexes) {
    //     using operand = typename spec::operand;
    //     constexpr std::size_t n_samples_per_operand = spec::n_samples_per_operand;
    ...
    //     LoadOperandsWithOffset<spec, n_samples_per_operand>::call(row_x, in_real, ind_xy);
    //     LoadOperandsWithOffset<spec, n_samples_per_operand>::call(row_u, in_real, ind_uv);
    //     ComputeTranspose<spec, n_samples_per_operand, n_samples_per_operand>::call(row_y, row_x);
    //     ComputeTranspose<spec, n_samples_per_operand, n_samples_per_operand>::call(row_v, row_u);
    //     StoreOperandsWithOffset<spec, n_samples_per_operand>::call(out_real, AlternateOut<spec, n_samples_per_operand>::call(row_x, row_y), ind_uv);
    //     StoreOperandsWithOffset<spec, n_samples_per_operand>::call(out_real, AlternateOut<spec, n_samples_per_operand>::call(row_u, row_v), ind_xy);

    //     LoadOperandsWithOffset<spec, n_samples_per_operand>::call(row_x, in_imag, ind_xy);
    //     LoadOperandsWithOffset<spec, n_samples_per_operand>::call(row_u, in_imag, ind_uv);
    //     ComputeTranspose<spec, n_samples_per_operand, n_samples_per_operand>::call(row_y, row_x);
    //     ComputeTranspose<spec, n_samples_per_operand, n_samples_per_operand>::call(row_v, row_u);
    //     StoreOperandsWithOffset<spec, n_samples_per_operand>::call(out_imag, AlternateOut<spec, n_samples_per_operand>::call(row_x, row_y), ind_uv);
    //     StoreOperandsWithOffset<spec, n_samples_per_operand>::call(out_imag, AlternateOut<spec, n_samples_per_operand>::call(row_u, row_v), ind_xy);
    // }
    ...
    //     BitRevPermPlan() : type(BitRevPermPlanType::indexes_mat_is_large_square) {}
    // };
    ...
    //         plan.type = BitRevPermPlanType::n_indexes_equals_base_size_sqr;
    //     } else if (n_indexes == 2 * base_size * base_size) {
    //         plan.type = BitRevPermPlanType::n_indexes_equals_2_base_size_sqr;
    //     } else if (n_indexes == 4 * base_size * base_size) {
    //         plan.type = BitRevPermPlanType::n_indexes_equals_4_base_size_sqr;
    //     } else if (n_indexes == 8 * base_size * base_size) {
    //         plan.type = BitRevPermPlanType::n_indexes_equals_8_base_size_sqr;
    //     } else if (indexes_mat_is_large_square) {
    //         plan.type = BitRevPermPlanType::indexes_mat_is_large_square;
    //     } else {
    //         plan.type = BitRevPermPlanType::indexes_mat_is_large_nonsquare;
    //     }
    ...
    // ) {
    //     using operand = typename spec::operand;
    //     constexpr size_t base_size = spec::n_samples_per_operand;
    //     BitRevPermPlanType plan_type = plan.type;
    ...
    //     switch (plan_type){
    //     case BitRevPermPlanType::n_indexes_equals_base_size_sqr:
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data);
    //         return;

    //     case BitRevPermPlanType::n_indexes_equals_2_base_size_sqr:
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data);
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + base_size);
    //         return;

    //     case BitRevPermPlanType::n_indexes_equals_4_base_size_sqr:
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data);
    //         transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + base_size);
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + 3 * base_size);
    //         return;

    //     case BitRevPermPlanType::n_indexes_equals_8_base_size_sqr:
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data);
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + base_size);
    //         transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + 2 * base_size);
    //         transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + 4 * base_size);
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + 6 * base_size);
    //         transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + 7 * base_size);
    //         return;
    ...
    //         for (std::size_t streak_len : off_diagonal_streak_lens) {
    //             for (std::size_t off_diagonal_id = 0; off_diagonal_id < streak_len; ++off_diagonal_id) {
    //                 transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + transpose_id * base_size);
    //                 transpose_id += 2;
    //             }

    //             transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + transpose_id * base_size);
    //             transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 1) * base_size);
    //             transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 3) * base_size);
    ...
    //             for (std::size_t off_diagonal_id = 0; off_diagonal_id < streak_len; ++off_diagonal_id) {
    //                 transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + transpose_id * base_size);
    //                 transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 2) * base_size);
    //                 transpose_id += 4;
    //             }

    //             transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + transpose_id * base_size);
    //             transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 1) * base_size);
    //             transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 2) * base_size);
    //             transpose_off_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 4) * base_size);
    //             transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 6) * base_size);
    //             transpose_diagonal<spec>(out_real, out_imag, in_real, in_imag, plan_indexes_data + (transpose_id + 7) * base_size);
    ...
    //     switch (type) {
    //         case BitRevPermPlanType::n_indexes_equals_base_size_sqr: return "n_indexes_base_size_sqr";
    //         case BitRevPermPlanType::n_indexes_equals_2_base_size_sqr: return "n_indexes_2_base_size_sqr";
    //         case BitRevPermPlanType::n_indexes_equals_4_base_size_sqr: return "n_indexes_4_base_size_sqr";
    //         case BitRevPermPlanType::n_indexes_equals_8_base_size_sqr: return "n_indexes_8_base_size_sqr";
    //         case BitRevPermPlanType::indexes_mat_is_large_square: return "indexes_mat_is_large_square";
    //         case BitRevPermPlanType::indexes_mat_is_large_nonsquare: return "indexes_mat_is_large_nonsquare";
    ...
    template<typename spec>
    void cobra(typename spec::sample* signal_real, typename spec::sample* signal_imag, typename spec::sample* transform_real, typename spec::sample* transform_imag, typename spec::sample* work_real, typename spec::sample* work_imag,
        ...
        using operand = typename spec::operand;
        using sample = typename spec::sample;
        constexpr std::size_t k_N_SAMPLES_PER_OPERAND = spec::n_samples_per_operand;
        ...
        for (
            ...
            for (
                ...
                for (
                    ...
                    operand store_real;
                    operand store_imag;
                    spec::load(store_real, A_p_real + c);
                    spec::load(store_imag, A_p_imag + c);
                    spec::store(T_p_real + c, store_real);
                    spec::store(T_p_imag + c, store_imag);
                }
            }
